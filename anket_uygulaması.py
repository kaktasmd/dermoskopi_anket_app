...
def upsert_lock(ehash, participant_id, completed, year, university, image_order_json):
    try:
        ...
        st.error(f"Oturum kayıt hatası: {e}")
...
def append_response(row):
    try:
        ...
        st.error(f"Cevabınız kaydedilirken hata oluştu (Lütfen tekrar deneyin): {e}")
        return False
        ...
        if not (year and university.strip() and email.strip()):
            ...
            with st.spinner("Katılımcı durumu kontrol ediliyor…"):
                ...
                if status.get("completed"):
                    ...
                    # Eğer kişi daha önce girip, şimdi farklı bir kıdem/üniversite girerse uyar ve durdur
                    if (stored_year and stored_year != year) or (stored_university and stored_university.lower() != university.strip().lower()):
                        st.error(
                            ...
                            "Lütfen tutarlılık için bilgilerinizi ilk girişinizdeki gibi düzeltip tekrar 'Devam Et'e basın."
                        )
                    else:
                        manifest = load_manifest()
                        by_key = {build_key(it): it for it in manifest}

                        # Veritabanından kişinin özel soru sırasını çekiyoruz
                        order_keys = json.loads(status.get("image_order") or "[]")
                        answered = set(status.get("answered_images", []))

                        # Henüz cevaplanmamış soruları kişinin kendi sırasında filtreliyoruz
                    ...
                    # YENİ KATILIMCI: Görüntüleri karıştır ve sırayı oluştur
                    manifest = load_manifest()
                    ...
                    upsert_lock(ehash, participant_id, False, year, university.strip(), order_json)
                    st.session_state.screen = "survey"
                    st.rerun()
...
# ─────────────────────────────────────────────────────────────────
    ...
    # Tüm sorular bittiyse bitiş ekranına yönlendir
    ...
    if st.button(btn_label, use_container_width=True, disabled=(choice is None)):
        ...
        # append_response True dönerse (yani kayıt başarılıysa) soruyu ilerletir
...
# ─────────────────────────────────────────────────────────────────
    ...
    st.success("Teşekkürler! 150 değerlendirmeniz başarıyla kaydedildi.")
